...
This guide is for people who want to use more than one Codex account in Jass Code.
...
In Jass Code Settings, name it something obvious:
...
In Jass Code Settings, add another Codex provider:
...
Jass Code shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Jass Code treats it
