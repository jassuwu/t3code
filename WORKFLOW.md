...
3. Extracts `T3 Code (Alpha).app` from the zip, quits the running app, replaces `/Applications/T3 Code (Alpha).app`, clears macOS quarantine, relaunches.
...
**Build succeeds but installed app still shows old behavior:** macOS Gatekeeper occasionally caches bundle metadata. Force a fresh launch: `killall 'T3 Code (Alpha)' 2>/dev/null ; open '/Applications/T3 Code (Alpha).app'`.
